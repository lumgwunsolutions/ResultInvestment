...
public class AddCorporateUserActivity extends ParentActivity implements View.OnClickListener {
    TextView headername;
    ImageView ic_back;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            e.printStackTrace();
            Util.ShowToast(AddCorporatePaymentMethodeActivity.this,
        ...
        switch (v.getId()) {
            ...
            case R.id.btnAddPayment:
                if (edt_corpoid.getText().toString().length() == 0) {
                    ...
                    Util.ShowToast(AddCorporatePaymentMethodeActivity.this, "Enter email
                            id");
                } else if
                (!Util.isEmailValid(edt_corpoid.getText().toString())) {
                    ...
                    try {
                        ...
                        jsonObject.put("regId", PackageInstaller.Session.getRegID(AddCorporatePaymentMethodeActivity.this));

                        jsonObject.put("countryCode", PackageInstaller.Session.getCc(AddCorporatePaymentMethodeActivity.this));
                        ...
                        jsonObject1.put("customerId",
                                PackageInstaller.Session.getUserID(AddCorporatePaymentMethodeActivity.this));
                        if (Constants.loginDetails != null) {
                            if
                            (Constants.loginDetails.facebookId.length() > 0) {
                                jsonObject.put("authKeyFb",
                                        Constants.loginDetails.facebookId);
                                jsonObject.put("signInVia", "F");
                            } else if
                            (Constants.loginDetails.googleId.length() > 0) {
                                jsonObject.put("authKeyG",
                                        Constants.loginDetails.googleId);
...
//                        gateway User Id – email for paypal
                        ...
                        jsonObject1.put("email",
                                edt_corpoid.getText().toString());

                        jsonObject1.put("pin",edt_pin.getText().toString().trim());
                        jsonObject.put("paymentList", new
    ...
    public void CallAPI(JSONObject params) {
        if (Util.isNetworkConnected(AddCorporatePaymentMethodeActivity.this)) {
            try {
                ...
                progressdialog.show();
                if (isforaddpayment)
                    new CallAPI(ADDPAYMENT, "ADDPAYMENT", params,
                            AddCorporatePaymentMethodeActivity.this, GetDetails_Handler, true);
                else
                    new CallAPI(VERIFYEMAILID, "ADDPAYMENTUSER",
                            params, AddCorporatePaymentMethodeActivity.this, GetDetails_Handler,
                            true);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.getData().getBoolean("flag")) {
                if (msg.getData().getInt("code") == SUCCESS) {
                    ...
                    if (isforaddpayment) {
                        ...
                        try {
                            JSONObject jsonObject = new
                                    JSONObject(msg.getData().getString("responce"));

                            progressdialog.dismissanimation(ProgressDialogView.ERROR);
                            if(jsonObject.getString("rembursed")== "1") {

                                PackageInstaller.Session.setUserID(AddCorporatePaymentMethodeActivity.this,
                                        jsonObject.getString("customerId"));

                                PackageInstaller.Session.setEmailId(AddCorporatePaymentMethodeActivity.this,
                                        jsonObject.getString("email"));

                                PackageInstaller.Session.setLoginStatus(AddCorporatePaymentMethodeActivity.this, true);

                                PackageInstaller.Session.setAllInfo(AddCorporatePaymentMethodeActivity.this,
                                        msg.getData().getString("responce"));
                                Intent intent = new
                                        Intent(AddCorporatePaymentMethodeActivity.this,
                    ...
                    if
                    (Util.isNetworkConnected(AddCorporatePaymentMethodeActivity.this)) {
