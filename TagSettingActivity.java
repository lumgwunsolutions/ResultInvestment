...
public class TagSettingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // wrap for dragging
        mWrappedAdapter =
                mRecyclerViewDragDropManager.createWrappedAdapter(myItemAdapter);

        final GeneralItemAnimator animator = new
        ...
        title.setText((RecordManager.TAGS.size() - 2) +
                mContext.getResources().getString(R.string.tag_number));
    ...
    public class SaveTags extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            for (int i = 0; i < myItemAdapter.getTags().size(); i++) {
                RINARecordManager.TAGS.set(i + 2,
                        myItemAdapter.getTags().get(i));
                RINARecordManager.TAGS.get(i + 2).setWeight(i);
            }
            for (int i = 2; i < RINARecordManager.TAGS.size(); i++) {
                RINARecordManager.updateTag(RINARecordManager.TAGS.get(i));
            }
            return null;
        }
        @Override
