...
public class MySwipeableItemAdapter
    ...
    private List<InvestmentPackage> records;
    ...
    public MySwipeableItemAdapter(Context inContext,
                                  List<InvestmentPackage> records, final OnItemDeleteListener
                                          onItemDeleteListener, OnItemClickListener onItemClickListener) {
        mContext = inContext;
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        final View v =
                inflater.inflate(R.layout.account_book_list_view_item, parent, false);
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        InvestmentPackage record = records.get(tPosition);
        holder.tagImage.setImageResource(
                RinaUtil.GetTagIcon(record.getTag()));
        holder.date.setText(record.getCalendarString());
        holder.money.setText(String.valueOf((int) record.getMoney()));
        holder.date.setTypeface(RinaUtil.typefaceLatoLight);
        holder.money.setTypeface(RinaUtil.typefaceLatoLight);
        holder.money.setTextColor(ContextCompat.getColor(Application.getAppContext(),
                R.color.my_blue));
        holder.index.setText((position + 1) + "");
        holder.index.setTypeface(RinaUtil.typefaceLatoLight);
        holder.remark.setText(record.getRemark());
        holder.remark.setTypeface(RinaUtil.typefaceLatoLight);
    ...
    @Override
    public int onGetSwipeReactionType(MyViewHolder holder, int
            position, int x, int y) {
        return Swipeable.REACTION_CAN_SWIPE_BOTH_H;
    }

    @Override
    public void onSetSwipeBackground(MyViewHolder holder, int
            position, int type) {
        int bgRes = 0;
        switch (type) {
    ...
    @Override
    public SwipeResultAction onSwipeItem(MyViewHolder holder, final
    int position, int result) {

        switch (result) {
            // swipe right
            case Swipeable.RESULT_SWIPED_RIGHT:
                if (pinned.get((int)records.get(records.size() - 1 -
                        position).getId())) {
    ...
    private static class SwipeLeftResultAction extends
        ...
        protected void onPerformAction() {
            ...
            if (!pinned.get((int)RecordManager.SELECTED_RECORDS.get(
                    RecordManager.SELECTED_RECORDS.size() - 1 -
                            mPosition).getId())) {
                pinned.put((int)RecordManager.SELECTED_RECORDS.get(
                        RecordManager.SELECTED_RECORDS.size() - 1 -
                                mPosition).getId(), true);
                mSetPinned = true;
    ...
    public static class SwipeRightResultAction extends
        ...
        SwipeRightResultAction(MySwipeableItemAdapter adapter, int
                position, OnItemDeleteListener onItemDeleteListener) {
            mAdapter = adapter;
            mPosition = position;
            this.onItemDeleteListener = onItemDeleteListener;
        }
        ...
        protected void onPerformAction() {
            super.onPerformAction();
            if (RinaUtil.backupCoCoinRecord != null) {

                RecordManager.deleteRecord(RinaUtil.backupInvestmentPackage, true);
            }
            RinaUtil.backupCoCoinRecord = null;
            RinaUtil.backupCoCoinRecord
                    =
                    RecordManager.SELECTED_RECORDS.get(RecordManager.SELECTED_RECORDS.size()
                            - 1 - mPosition);
            RecordManager.SELECTED_RECORDS.remove(RecordManager.SELECTED_RECORDS.size()
                    - 1 - mPosition);
            RecordManager.SELECTED_SUM -=
                    RinaUtil.backupInvestmentPackage.getMoney();
