...
public class DialogTagChooseGridViewAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.tagImage.setImageResource(
                CoCoinUtil.GetTagIcon(RecordManager.TAGS.get(position
                        + 2).getId()));
        holder.tagName.setText(CoCoinUtil.GetTagName(RecordManager.getInstance(mContext).TAGS.get(position
                + 2).getId()));
