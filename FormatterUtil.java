...
public class FormatterUtil {
    ...
    public static final long NOW_TIME_RANGE =
            DateUtils.MINUTE_IN_MILLIS * 5; // 5 minutes
    ...
    public static CharSequence getRelativeTimeSpanStringShort(Context
                                                                      context, long time) {
        long now = System.currentTimeMillis();
        long range = Math.abs(now - time);
    ...
    private static CharSequence formatDuration(Context context, long
            range, long time) {
        ...
        if (range >= DateUtils.WEEK_IN_MILLIS + DateUtils.DAY_IN_MILLIS) {
            ...
            final int days = (int) ((range + (DateUtils.WEEK_IN_MILLIS
            ...
            final int days = (int) ((range + (DateUtils.DAY_IN_MILLIS
            ...
            final int hours = (int) ((range +
                    (DateUtils.HOUR_IN_MILLIS / 2)) / DateUtils.HOUR_IN_MILLIS);
            return
                    String.format(res.getString(R.string.duration_hours_shortest), hours);
        } else if (range >= NOW_TIME_RANGE) {
            final int minutes = (int) ((range +
                    (DateUtils.MINUTE_IN_MILLIS / 2)) / DateUtils.MINUTE_IN_MILLIS);
            return
                    String.format(res.getString(R.string.duration_minutes_shortest),
                            minutes);
        } else {
    ...
    private static String shortFormatEventDay(Context context, long time) {
        ...
        return DateUtils.formatDateRange(context, f, time, time,
                flags).toString();
    }
    }
